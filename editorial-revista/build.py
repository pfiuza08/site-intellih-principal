#!/usr/bin/env python3
"""Gera as páginas estáticas da revista a partir de conteudo/artigos.json.
Executar: python3 build.py
...
ARTICLES = json.loads((BASE / 'conteudo' / 'artigos.json').read_text(encoding='utf-8'))
EDITORIAS = {
...
def picture(a, prefix='./', tag=False):
    sticker='<span class="image-tag">'+h(a['formato'])+'</span>' if tag else ''
    return f'<div class="image-box"><img src="{asset("img/"+a["imagem"],prefix)}" alt="{h(a["credito_imagem"])}" width="1440" height="900" loading="lazy">{sticker}</div>'
...
def article_page(a):
    ...
    body=f'''<div class="wrap article-hero"><div class="breadcrumbs"><a href="../index.html">Revista</a> / <a href="../editorias/{'ciencia' if a['categoria']=='Ciência' else 'tecnologia'}.html">{h(a['categoria'])}</a> / {h(a['editoria'])}</div><span class="eyebrow">{h(a['categoria'])} · {h(a['formato'])}</span><h1>{h(a['titulo'])}</h1><p class="dek">{h(a['subtitulo'])}</p><div class="article-meta"><span>Redação Intellih · texto de demonstração</span><span>Preparado em {datetime.strptime(a['data'],'%Y-%m-%d').strftime('%d/%m/%Y')}</span><span>{h(a['leitura'])} de leitura</span></div></div>
<div class="article-media">{picture(a,prefix)}<p class="credit">{h(a['credito_imagem'])}</p></div>
...
<section class="article-footer"><div class="wrap"><h2>Gostou de descobrir mais?</h2><a class="pill-link" href="../index.html">Voltar à revista →</a></div></section>'''
    (SITE/'artigos'/f'{a["slug"]}.html').write_text(doc(a['titulo'],a['subtitulo'],body,prefix,active='Revista'),encoding='utf-8')
...
def validate():
    ...
    for a in ARTICLES:
        ...
        slugs.add(a['slug'])
        if a['imagem'] not in ('lua-impacto.svg','buraco-negro.svg','ia-modelos.svg'):raise ValueError('Imagem ausente')
        for s in a['fontes']:
