...
- **Nem toda IA é generativa:** texto revisado, aprovado e incorporado à prévia. A editoria da revista chama-se **Inteligência Artificial**; a expressão usada anteriormente como rótulo da editoria permanece reservada aos canais de vídeo e redes sociais. O rótulo de formato da matéria passou a **Explicação**, sem alteração do seu corpo aprovado.

O artigo sobre previsão/classificação da chuva foi arquivado em `editorial-revista/rascunhos/` para uma edição futura. A fonte editorial é `editorial-revista/conteudo/noticias.json` mais `editorial-revista/conteudo/artigos.json`; `python3 editorial-revista/build.py` gera homepage, quatro artigos e seis editorias. A antiga URL da editoria direciona para `/revista/editorias/inteligencia-artificial.html`, evitando links quebrados. As automações temporárias foram removidas.
...
Antes de publicar, conferir ilustrações e direitos, datas reais de publicação, metadados definitivos, links e visual desktop/mobile. Somente com autorização expressa será possível planejar a entrada no domínio de produção.
